...
class DocumentHistory(Base):
    ...
    ocr_executado = Column(Boolean, default=False, nullable=False)
    ocr_status = Column(String(50), nullable=True)
    ocr_idioma = Column(String(20), nullable=True)
    ocr_paginas_processadas = Column(Integer, nullable=True)
    ocr_tempo_ms = Column(Integer, nullable=True)
    ocr_erro = Column(Text, nullable=True)
    ocr_executado_em = Column(DateTime(timezone=False), nullable=True)
